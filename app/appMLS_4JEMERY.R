...
setwd("D:/git/PacificCommunity/ofp-sam/ofp-sam-mls-2024-shiny/app")
...
# default_models <- c("C02CatchCond8") #c("01Diag2020")
default_fishery <- "LL.1"
rec_units <- 1000000
sb_units <- 1000
copyright.year <- 2024
shiny.title <- "MLS stepwise 2024"
...
# ll_offset[, Recruitment := sprintf("%.2f",Recruitment)]
...
ui <- dashboardPage(
  ...
  sidebar = dashboardSidebar(
    ...
    conditionalPanel(condition="input.sidebarmenu == 'diagnostics' || input.sidebarmenu == 'fittodata' || input.sidebarmenu == 'modeloutput' || input.sidebarmenu == 'stockstatus'",
                     ...
                     # checkboxGroupInput(inputId="model_select", label="Select models", choiceNames=all_models, choiceValues=all_models, selected=default_models, inline = FALSE),
      ...
      tabItem(tabName="stockstatus", h2("Stock status"),
              fluidRow(
                box(title="Recruitment", solidHeader=TRUE, collapsible=TRUE, collapsed=start_collapsed, status="primary", width=12,
                    # Area selector
                    column(6, radioButtons(inputId="area_select_recruitment", label="Region selector", choiceNames=c("Separate", "Combined"), choiceValues=c("separate", "combined"), selected="separate", inline=TRUE)),
                    # Scale selector
              ...
              fluidRow(
                box(title="Depletion", solidHeader=TRUE, collapsible=TRUE, collapsed=start_collapsed, status="primary", width=8,
                    # Area selector
                    radioButtons(inputId="area_select_sbsbf0", label="Region selector", choiceNames=c("Separate", "Combined"), choiceValues=c("separate", "combined"), selected="separate", inline=TRUE),
              ...
              fluidRow(
                box(title="Spawning potential", solidHeader=TRUE, collapsible=TRUE, collapsed=start_collapsed, status="primary", width=8,
                    # Area selector
                    column(6, radioButtons(inputId="area_select_sb", label="Region selector", choiceNames=c("Separate", "Combined"), choiceValues=c("separate", "combined"), selected="separate", inline=TRUE)),
                    # Scale selector
              ...
              fluidRow(
                box(title="Unfished biomass", solidHeader=TRUE, collapsible=TRUE, collapsed=start_collapsed, status="primary", width=8,
                    # Area selector
                    column(6, radioButtons(inputId="area_select_sbf0", label="Region selector", choiceNames=c("Separate", "Combined"), choiceValues=c("separate", "combined"), selected="separate", inline=TRUE)),
                    # Scale selector
...
server <- function(input, output){
  ...
  output$plot_catch_size_dist <- renderPlot({
    ...
    p <- p + geom_line(data=pdat, aes(y=pred, colour=model), linewidth=0.75)
  ...
  output$plot_tag_returns_time <- renderPlot({
    ...
    p <- p + geom_line(aes(y=recap.pred, colour=model), na.rm=TRUE, linewidth=0.75)
    p <- p + scale_colour_manual("Model", values=model_cols)
  ...
  output$plot_tag_attrition <- renderPlot({
    ...
    p <- p + geom_line(aes(y=recap.pred, colour=model), linewidth=0.75)
    p <- p + scale_colour_manual("Model", values=model_cols)
    if(facet == "program"){
  ...
  output$plot_tag_attrition_residuals <- renderPlot({
    ...
    p <- p + geom_smooth(aes(colour=model), method='loess', formula='y~x', na.rm=TRUE, se=FALSE, linewidth=0.75)
  ...
  output$plot_srr <- renderPlot({
    ...
    # The fitted model
    p <- p + geom_line(data=fdat, aes(x=sb/sb_units, y=rec/rec_units, colour=model), linewidth=0.75)
    p <- p + scale_fill_manual("Model", values=model_cols)
    p <- p + scale_colour_manual("Model", values=model_cols)
  ...
  output$plot_growth <- renderPlot({
    ...
    p <- p + geom_line(aes(y=length, colour=model), linewidth=0.75)
    p <- p + scale_colour_manual("Model", values=model_cols)
    p <- p + scale_fill_manual("Model", values=model_cols)
  ...
  output$plot_growth_transposed <- renderPlot({
    ...
    p <- p + geom_line(aes(x=length, colour=model), linewidth=0.75)
    p <- p + scale_colour_manual("Model", values=model_cols)
    p <- p + scale_fill_manual("Model", values=model_cols)
